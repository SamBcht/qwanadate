...
These files are automatically detected after entering the **Tree ID** and selecting the base directory.
...
The user must also specify the **unit scaling** of the TRW values (e.g. mm, µm).
...
If no match is found in the dendro file, the app allows manual selection.
...
5. Run crossdating
6. Switch to the **Manual Crossdating** window and correct the crossdating
   Anatomical segments can be moved to the desired calendar year by clicking on the corresponding bar
   on the profile plot.
   Alternatively they can be moved by using the `Crossdating` drop-down menu on the left
7. Create the anatomical chronology and verify the alignment under the `Validation` window
8. Specify the place to store the output file and save with the drop-down menu on the left
